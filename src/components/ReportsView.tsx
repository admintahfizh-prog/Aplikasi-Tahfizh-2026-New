...
export const ReportsView: React.FC<ReportsViewProps> = ({
  ...
  const [reportType, setReportType] = useState<'hafalan' | 'ummi' | 'rekap_nilai' | 'raport_kelas'>('hafalan');
    ...
    <div className="space-y-6 animate-in fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white p-5 rounded-xl border border-slate-200 shadow-xs no-print">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            Cetak laporan resmi, rekapitulasi nilai per kelas, dan ekspor data ke Excel/CSV
          </p>
        </div>
      ...
      <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs space-y-3 no-print">
        <div className="flex flex-wrap items-center gap-2 border-b border-slate-100 pb-3">
          {[
            { id: 'hafalan', label: '1. Rekap Capaian Hafalan Al-Qur\'an', icon: BookOpen },
            { id: 'ummi', label: '2. Rekap Pembelajaran Metode Ummi', icon: BookMarked },
            { id: 'rekap_nilai', label: '3. Rekapitulasi Nilai & Evaluasi', icon: Award },
            { id: 'raport_kelas', label: '4. Buku Induk Tahfizh Kelas', icon: Layers },
          ].map((tab) => {
            const Icon = tab.icon;
      ...
      <div className="bg-white p-8 rounded-xl border border-slate-200 shadow-xs space-y-6 print:p-0 print:border-none print:shadow-none">
        ...
        <div className="pt-8 grid grid-cols-2 text-center text-xs">
          <div>
            ...
            <p className="font-bold text-slate-800">Kepala Sekolah SMPIA21</p>
            <div className="h-16"></div>
            <p className="font-bold text-slate-900 underline">Dr. H. Muhammad Ilyas, M.Pd.I.</p>
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-500">NIP. 19850314 201001 1 003</p>
          </div>
        </div>
